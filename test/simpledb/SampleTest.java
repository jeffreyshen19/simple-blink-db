...
public class SampleTest {
    ...
    public void testSampleSelection() throws Exception {
        ...
        p = new Predicate(2, Predicate.Op.GREATER_THAN_OR_EQ, new IntField(2011));
        filter = new Filter(p, seqscan);
        query = new Aggregate(filter, 2, -1, Aggregator.Op.COUNT);
        queryColumnSet = new QueryColumnSet(query);
        //qcs in stratified samples
        ...
        // SELECT SUM(quantity) FROM table WHERE year >=2013
        p = new Predicate(2, Predicate.Op.GREATER_THAN_OR_EQ, new IntField(2011));
        filter = new Filter(p, seqscan);
        query = new Aggregate(filter, 1, -1, Aggregator.Op.SUM);
        queryColumnSet = new QueryColumnSet(query);
