...
public class JoinOptimizer {
    ...
    private final static double PERCENT_EMITTED = 0.3;
    ...
    public static int estimateTableJoinCardinality(Predicate.Op joinOp,
        ...
        if(joinOp == Predicate.Op.EQUALS) {
            // If one of the join attrs is primary key, card cannot be greater than cardinality of non-primary key attr
            if(t1pkey) return card2;
            else if(t2pkey) return card1;
            // Otherwise, it could be anything: let's say its just the larger table
            else return Math.max(card1, card2);
        }
        // Range scan: output is proportion of the cross product, assuming a fixed percent of tuples are emitted
        else {
            return (int) (PERCENT_EMITTED * card1 * card2);
        }
    }
    ...
    public List<LogicalJoinNode> orderJoins(
        ...
        for(int i = 1; i <= joins.size(); i++) {
            ...
            for(Set<LogicalJoinNode> s : subsets) {
                CostCard bp = null;
                Set<LogicalJoinNode> s_prime = new HashSet<>(s); 
                for (LogicalJoinNode joinToRemove : s) {
                    s_prime.remove(joinToRemove);
                    
                    double bestCostSoFar = bp == null ? Double.POSITIVE_INFINITY : bp.cost;
                    CostCard cc = computeCostAndCardOfSubplan(stats, filterSelectivities, joinToRemove, s_prime, bestCostSoFar, pc);
                    
                    if(bp == null || (cc != null && cc.cost < bp.cost)) bp = cc;
                    
                    s_prime.add(joinToRemove);
                }
                if(bp != null) pc.addPlan(s_prime, bp.cost, bp.card, bp.plan);
            }
